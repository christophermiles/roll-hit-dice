...
export function parseHitDice(expression: string, strict = false): {
    diceCount: DiceCount
    dieType: DieType
    modifier: Modifier
} {
    const match = expression.match(strict ? HIT_DICE_REGEX_STRICT : HIT_DICE_REGEX)
...
export default function rollHitDice(expression: string, strict = false): HitPointResults {
    const { diceCount, dieType, modifier } = parseHitDice(expression, strict)
